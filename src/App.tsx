...
import {Bubble, CodeHighlighter, Conversations, Mermaid, Sender, Sources, Think, XProvider} from "@ant-design/x";
...
const markdownConfig: XMarkdownProps["config"] = {
    gfm: true,
    extensions: Latex(),
};

const markdownIconMap: Record<MarkdownIconName, ComponentType<{className?: string; title?: string}>> = {
    bulb: BulbOutlined,
...
const MarkdownCode = ({
    ...
    return (
        <CodeHighlighter className="markdown-code-highlighter" lang={language || "text"}>
            {code}
        </CodeHighlighter>
    );
};

const MarkdownPre = ({children}: XMarkdownComponentProps) => <>{children}</>;

const MarkdownThink = ({
    children,
    streamStatus,
    title,
}: XMarkdownComponentProps<{title?: string}>) => (
    <Think
        title={title || "思考"}
        loading={streamStatus === "loading"}
        defaultExpanded={false}
        blink={streamStatus === "loading"}
    >
        {children}
    </Think>
);

const MarkdownMermaid = ({children}: XMarkdownComponentProps) => (
    <Mermaid>{childrenToText(children)}</Mermaid>
);
...
const MarkdownIcon = ({
    ...
    const Icon = markdownIconMap[name || type || icon || "info"] || InfoCircleOutlined;

    return <Icon className="markdown-icon" title={title}/>;
};
...
const MarkdownSources = ({
    ...
    <Sources title={title || "来源"} defaultExpanded={false}>
        {children}
    </Sources>
);
...
const markdownProps: Partial<XMarkdownProps> = {
    className: "x-markdown-light assistant-markdown",
    components: markdownComponents,
    config: markdownConfig,
    dompurifyConfig: {
        ADD_ATTR: ["description", "icon", "name", "title", "type", "url"],
    },
...
function App() {
    ...
    const [projectDirCreating, setProjectDirCreating] = useState(false);
    ...
    const insertSessionInGroup = (session: ChatSession, projectRoot: string) => {
        setSessions((current) => {
            const index = current.findIndex(
                (item) => getSessionProjectRoot(item) === projectRoot,
            );
            ...
            return [...current.slice(0, index), session, ...current.slice(index)];
        });
    };

    const createProjectSession = async (projectDir: string) => {
        ...
        insertSessionInGroup(
            ...
            sessionContext.workspace.project_root,
        );
    ...
    const handleCreateSession = async () => {
        const projectRoot = activeSession ? getSessionProjectRoot(activeSession) : "/";
        ...
        try {
            await createProjectSession(projectRoot);
        } catch (error) {
            setBootError(getErrorMessage(error));
        }
    };
    ...
    const handleCreateProjectSession = async () => {
        if (projectDirCreating) {
            return;
        }

        setProjectDirCreating(true);
        ...
        try {
            ...
            if (typeof selected === "string") {
                await createProjectSession(selected);
            }
        } catch (error) {
            setBootError(getErrorMessage(error));
        } finally {
            setProjectDirCreating(false);
        }
    };
    ...
    const createSessionMenu: MenuProps = {
        items: [
            {key: "chat", label: "新会话"},
                ...
                <main className="app-shell">
                    <aside className="sidebar">
                        <div className="sidebar-header">
                            ...
                            <Dropdown menu={createSessionMenu} trigger={["click"]}>
                                ...
                                items={[
                                    {
                                        key: "conversations",
                                        label: (
                                            <span className="conversation-section-name">对话</span>
                                        ),
                                        extra: (
                                            <Button
                                        ...
                                        children: groupedConversationItems.conversationItems.length ? (
                                            <Conversations
                                    ...
                                    items={[
                                        {
                                            key: "projects",
                                            label: (
                                                <span className="conversation-section-name">项目</span>
                                            ),
                                            extra: (
                                                <Button
                                                    className="title-add-button"
                                            ...
                                            children: groupedConversationItems.projectGroups.map(
                                                (group) => (
                                                    <Collapse
                                                        className="project-conversation-group"
                                                        key={group.key}
                                                        bordered={false}
                                    ...
                                    role={{
                                        ...
                                        assistant: {
                                            ...
                                            contentRender: (content, info) => {
                                                ...
                                                const currentMarkdownProps: Partial<XMarkdownProps> = {
                                                    ...markdownProps,
                                                    streaming: {
                                                ...
                                                return (
                                                    <div className="assistant-message">
                                                        {reasoningContent ? (
                                                            ...
                                                            >
                                                                <XMarkdown
                                                                    content={reasoningContent} {...currentMarkdownProps} />
                                                            </Think>
                                                        ) : null}
                                                        {parts.length ? (
                                                            parts.map((part) =>
                                                        ...
                                                        {parts.length ? null : (
                                                            <XMarkdown content={String(content)} {...currentMarkdownProps} />
                                                        )}
                                                    </div>
                                                );
                                            },
